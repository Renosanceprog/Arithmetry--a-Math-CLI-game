...
void ask_Question(GameState *state, MathProblem *curProblem)
{
    ...
    if (state->qs % 10 == 0 && state->qs != 0) {
        *curProblem = generateBossProblem(state->choice, state->qs);
        printCentered(6, "!!! BOSS QUESTION !!!", "\e[38;5;196m"); 
...
char showGamemodes()
{
    // 1. Clear screen and set up the aesthetic
    system("cls");
    drawBorder();
    drawGameLogos(1); // Use the main hub logo!

    // 2. Define our menu
    char* options[] = {"Classic", "Addition Only", "Subtraction Only", "Multiplication Only", "Division Only", "Return to Hub"};
    
    // 3. Run the engine (Starts at row 12)
    int choice = runInteractiveMenu(12, "=== SELECT GAMEMODE ===", options, 6);
...
int loginUser()
{
    ...
    // 1. Clean UI Slate
    system("cls"); drawBorder(); drawGameLogos(1);
    ...
    do
    {
        clearInnerRow(14);
        clearInnerRow(15);
        if (valid != 0) printCentered(14, "Username not Found!", C_RED);
    ...
    do
    {
        clearInnerRow(16);
        clearInnerRow(17);
        if (valid != 0) printCentered(16, "Incorrect Password!", C_RED);
...
int registerUser() 
{
    ...
    if (getVisibleInput(new_user->userNames, 50) == -1) {
        free(new_user); // MEMORY LEAK PREVENTION
        return -1;
    }
...
int getVisibleInput(char *buffer, int max_len) 
{
    ...
    while (1) {
        ...
        if (c == 27) return -1; // Player pressed ESC!
        
        if (c == '\r' || c == '\n') {
            if (i == 0) continue; // Don't allow empty strings
            break;
        }
...
int start_Hub()
{
    while (1)
    {
        ...
        case 2: system("cls"); drawBorder(); showUserStats(); break;
        case 3: system("cls"); drawBorder(); drawGameLogos(4); if (showLeaderboard(showCategoryMenu()) == -1) goto avoidpause; break;
        case 4: return -1;
...
int showCategoryMenu()
{
    ...
    return runInteractiveMenu(12, "===== Categories =====", options, 7);
}
...
int showLeaderboard(int category) {
    system("cls"); drawBorder(); drawGameLogos(4);
...
void drawGameLogos(int screen_id) 
{
    switch (screen_id) 
    {
        ...
        case 5: // START GAME
        {
            ...
            for (int i = 0; i < 6; i++) {
                printCentered(4 + i, logo[i], C_CYAN);
            }
            break;
        }
        case 6: // GAME OVER
